...
const API_KEY_INTRO =
	'AI Review uses a Cursor API key for the inline "Ask AI" chat.';

const API_KEY_DETAIL =
	'Add one now? You can also set it later via the ' +
	'"gerrit.aiReview.apiKey" setting or the CURSOR_API_KEY ' +
...
async function ensureCursorApiKey(): Promise<void> {
	...
	const pick = await window.showInformationMessage(
		API_KEY_INTRO,
		{ modal: true, detail: API_KEY_DETAIL },
