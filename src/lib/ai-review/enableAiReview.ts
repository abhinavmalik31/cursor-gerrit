...
export async function enableAiReview(context: ExtensionContext): Promise<void> {
	...
	await config.update('gerrit.aiReview.checkoutBehavior', checkoutBehavior);
	...
	const mcpOk = await writeMcpConfig(context.extensionPath, credentials);
	if (!mcpOk) {
		void window.showWarningMessage(
...
async function verifyCursorCli(): Promise<boolean> {
	const { success, stdout } = await tryExecAsync('which cursor', {
		silent: true,
	});
